...
Сервер реализован на встроенном http.server (без внешних зависимостей),
использует ThreadingHTTPServer, чтобы один "упавший" запрос не мешал
обработке остальных.

Маршруты:
    GET  /users            -> список всех пользователей
    GET  /users/<id>       -> данные одного пользователя (404, если нет)
    POST /users/<id>/score -> {"score": <число>} прибавляется к текущему счёту

Любой другой маршрут -> 404.
Неподдерживаемый метод на известном маршруте -> 405.
"""
